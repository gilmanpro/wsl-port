...
class WslProvider:
    ...
    def clone(self, name: str, new_name: str) -> CommandResult:
        ...
        tmp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".tar", prefix="clone-tmp-")
    ...
    def metrics(self, name: str) -> Optional[DistroMetrics]:
        ...
        if not m.running:
            return m
        free = self._wsl_d(name, ["free", "-m"], timeout=30)
        if free.ok:
            for line in free.output.splitlines():
        ...
        nproc = self._wsl_d(name, ["nproc"], timeout=30)
        if nproc.ok and nproc.output.strip().isdigit():
            m.cpus = int(nproc.output.strip())
        up = self._wsl_d(name, ["cat", "/proc/uptime"], timeout=30)
        if up.ok and up.output.strip():
            m.uptime_s = int(float(up.output.split()[0]))
        return m
