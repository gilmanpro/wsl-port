...
def api_enable(ctx: typer.Context, port: int = typer.Option(8791, "--port")):
    if not (1024 <= port <= 65535):
        typer.echo(f"error: puerto {port} fuera de rango (1024-65535)", err=True)
        raise typer.Exit(2)
    cfg = ctx.obj.store.get()
...
def api_tokens(
    ...
    if action == "create":
        ...
        token_file = token_dir / f"token-{int(time.time())}.txt"
        token_file.write_text(token, encoding="utf-8")
        # Restringir permisos (solo el usuario actual puede leer/escribir)
        try:
            token_file.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass  # Windows: chmod parcial; ACL se gestiona por defecto

        typer.echo(f"Token creado y guardado en: {token_file}")
        typer.echo(
            "ADVERTENCIA: El token NO se muestra en pantalla por seguridad. "
            "Lea el archivo para obtenerlo y luego elimínelo. "
            "Ejemplo de uso: curl -H 'Authorization: Bearer <TOKEN>' http://localhost:8791/api/v1/health"
        )
        return
