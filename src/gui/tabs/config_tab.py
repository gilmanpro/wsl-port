...
class ConfigTab(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        ttk.Button(bar, text="🔄 Recargar", command=self.refresh).pack(side="left", padx=2)
        ttk.Button(bar, text="💾 Guardar (con backup)", command=self._save).pack(side="left", padx=2)
        ttk.Label(bar, text="  ~/.wslconfig — configuracion de la VM WSL2", foreground="#888").pack(side="left")
