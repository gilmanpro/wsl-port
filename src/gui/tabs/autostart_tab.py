...
class AutoStartTab(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        self.tree = make_tree(
            self,
            [("distro", "Distro", 180), ("delay", "Retraso (s)", 100), ("cmd", "Comando en Run", 480)],
        )
        ttk.Label(
            self,
        ...
        ttk.Button(actions, text="＋ Activar autoarranque...", command=self._enable).pack(side="left", padx=2)
        ttk.Button(actions, text="－ Desactivar", command=self._disable).pack(side="left", padx=2)
