...
Autocontenido (ctypes, CryptProtectData/CryptUnprotectData) para mantener
el core sin dependencias. Los valores NUNCA se imprimen; solo `check`
confirma existencia. Fuera de Windows: fallback XOR (solo dev/test).
"""
...
def _fallback_protect(data: bytes) -> bytes:
    key = os.environ.get("USERNAME", "wsl-manager-gui").encode()
